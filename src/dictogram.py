...
class Dictogram(dict):
    ...
    def __init__(self, path=None):
        ...
        if path is not None:
            ...
            with open(path, 'r') as f:
                ...
                self.word_count = len(some_words)
            # find all words
        ...
        # self.weights = [self.get(key, 0)/self.sum for key in self]
    ...
    @time_it
    def sample(self):
        index = self.sum * random()
        # for i in range(len(self.dictogram)):
        #     if self.dictogram.keys

        #     if compare < self.dictogram.items()[i]
...
def main():
    ...
    # print(histogram.frequency('boy'))
    print(histogram.sum)
    # print(histogram.sample())
    print(histogram.prob('boy'))
